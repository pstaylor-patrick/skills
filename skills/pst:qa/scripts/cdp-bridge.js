...
class ChromeLink {
  ...
  static async attach(port) {
    if (typeof globalThis.WebSocket === "undefined") {
      fail(
        "node-version",
        "Node 22+ required for native WebSocket. Current: " + process.version,
      );
    }
...
async function handleLaunch() {
  ...
  if (!browser)
    fail(
      "no-chrome",
      "Chrome or Chromium not found. Set CHROME_PATH env to override.",
    );
...
async function handleCapture() {
  ...
  try {
    switch (captureType) {
      ...
      case "screenshot": {
        const result = await link.send("Page.captureScreenshot", {
          format: "png",
        });
        if (args.save) {
      ...
      default:
        fail(
          "args",
          `Unknown capture type: ${captureType}. Use: dom, screenshot, metrics, url`,
        );
    }
  } finally {
    link.close();
...
async function handleTeardown() {
  ...
  if (args.stream) {
    ...
    try {
      process.kill(streamPid, "SIGTERM");
    } catch (e) {
      errors.push(`Stream PID ${streamPid}: ${e.message}`);
    }
  }
  ...
  if (args.profile) {
    try {
      ...
      errors.push(`Profile dir: ${e.message}`);
    }
  }
  ...
  if (args.output) {
    try {
      ...
      errors.push(`JSONL file: ${e.message}`);
    }
  }
